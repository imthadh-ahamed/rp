...
export default function VoiceChatbot() {
  ...
  const submitEvaluation = useCallback(async () => {
    addMessage(
      "bot",
      "⏳ Submitting your answers for AI analysis. Processing audio recordings with speech-to-text + ML scoring — this can take a few minutes. Please keep this tab open!"
    );
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Server returned ${res.status}: ${errText.slice(0, 200)}`
        );
      }
      const result = await res.json();
      localStorage.setItem("commEvalResult", JSON.stringify(result));
      setPhase("done");
      router.push("/soft-skills/com-results");
    } catch (err: unknown) {
      ...
      const userMsg = isNetworkErr
        ? "Could not reach the backend. Please make sure the backend server is running on port 8000, then exit and try again."
  ...
  const handleStart = async () => {
    ...
    addMessage(
      "bot",
      "Hi! I'm your Communication Evaluator. I'll ask you 4 questions — just speak naturally after you press the microphone button. Let's begin!"
    );
    await speak(
      "Hi! I'm your Communication Evaluator. I'll ask you 4 questions. Just speak naturally after you press the microphone button. Let's begin!"
    );
  ...
  const startRecording = async () => {
    ...
    mr.onstop = async () => {
      ...
      if (nextIdx < QUESTIONS.length) {
        ...
        addMessage(
          "bot",
          "Excellent! Analyzing your communication skills now. This may take a minute..."
        );
        await speak(
          "Excellent! Analyzing your communication skills now. This may take a minute."
        );
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex flex-col items-center justify-center p-4 md:p-8">
      <div className="w-full max-w-4xl mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-indigo-300 text-sm mt-1">
            AI-powered voice skill assessment
          </p>
        </div>
        {phase !== "intro" && (
          <button
      ...
      <div
        className="w-full max-w-4xl bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col"
            ...
            <div className="flex flex-col items-center justify-center h-full gap-6 py-10">
              ...
              <div className="text-center">
                ...
                <p className="text-indigo-300 text-sm max-w-md">
                  You&apos;ll be asked{" "}
                  <strong className="text-white">4 questions</strong>. Answer
                  each one by speaking into your microphone. The AI will
                  evaluate your{" "}
                  <strong className="text-white">
                    clarity, vocabulary, fluency, structure,
                  </strong>{" "}
                  and <strong className="text-white">confidence</strong>.
                </p>
              </div>
              <div className="flex gap-3 text-xs text-indigo-300/80 flex-wrap justify-center">
                {["Clarity", "Vocabulary", "Fluency", "Structure", "Confidence"].map(
                  (s) => (
                    <span key={s} className="px-3 py-1 bg-white/10 rounded-full">
                      {s}
                    </span>
                  )
                )}
                  ...
                  return (
                    <div
                      ...
                      className={`w-1.5 rounded-full transition-all ${
                        i < filled ? "bg-indigo-400" : "bg-white/10"
                      }`}
                      style={{ height: `${20 + Math.random() * 14}px` }}
                    />
                  );
                })}
                <span className="ml-2 text-indigo-300 text-xs tabular-nums">
            ...
            <div className="flex items-center gap-4">
              ...
              {!isRecording && (
                <p className="text-indigo-300/70 text-xs">
                  Press to answer Question {currentQ + 1}
                </p>
              )}
            </div>
