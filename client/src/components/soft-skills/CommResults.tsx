...
import { BookOpen, Award, TrendingUp, RotateCcw, ExternalLink } from "lucide-react";
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push("/soft-skills/communication")}
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="text-center">
          ...
          <p className="text-indigo-300 mt-2 text-sm">
            Powered by AI speech &amp; language analysis
          </p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="flex flex-col items-center gap-3">
              ...
              <span
                className={`px-4 py-1.5 rounded-full border text-sm font-semibold ${levelBg}`}
              >
                {result.level}
              </span>
            </div>
            <div className="flex-1 w-full space-y-4">
              <h2 className="text-white font-bold text-lg flex items-center gap-2">
                <TrendingUp size={18} className="text-indigo-400" /> Skill
                Breakdown
              </h2>
              {SCORE_KEYS.map((key) => (
                <ScoreBar
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {SCORE_KEYS.map((key) => (
            ...
            >
              <div
                className={`text-2xl font-black bg-gradient-to-br ${gradient} bg-clip-text text-transparent`}
              >
        ...
        {result.recommended_courses.length > 0 && (
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
            <h2 className="text-white font-bold text-lg flex items-center gap-2 mb-6">
              <BookOpen size={18} className="text-violet-400" /> Recommended
              Courses
            </h2>
              ...
              {result.recommended_courses.map((course, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`self-start px-2.5 py-1 rounded-full text-xs border font-medium ${platClass}`}
                    >
                      {course.platform}
                    </span>
                  </a>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <button
            onClick={() => {
              ...
              router.push("/soft-skills/communication");
            }}
