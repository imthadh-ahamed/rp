...
const ProbRecomm: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (!stored) {
        setError(
          "No evaluation results found. Please complete the evaluation first."
        );
      } else {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex items-center justify-center px-4">
        <div className="bg-white/5 border border-white/10 backdrop-blur-xl rounded-3xl p-10 max-w-md w-full text-center space-y-5">
          ...
          <button
            onClick={() => router.push("/soft-skills/problem-solving")}
  ...
  const scorePercent = Math.round(
    (Math.min(data.final_score, maxScore) / maxScore) * 100
  );
  const levelPercent =
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="flex flex-col items-center gap-3 flex-shrink-0">
              ...
              />
              <span
                className={`px-4 py-1.5 rounded-full border text-sm font-semibold ${lvl.badge}`}
              >
                {data.skill_level}
              </span>
              <p className="text-white/40 text-xs text-center max-w-[140px]">
                {lvl.desc}
              </p>
            </div>
            ...
            <div className="flex-1 w-full space-y-5">
              <h2 className="text-white font-bold text-lg flex items-center gap-2">
                <TrendingUp size={18} className="text-indigo-400" /> Performance
                Metrics
              </h2>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <button
            onClick={() => {
              ...
              router.push("/soft-skills/problem-solving");
            }}
